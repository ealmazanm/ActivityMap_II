...
void hungarian_print_matrixFloat(float** C, int rows, int cols) {
  int i,j;
  fprintf(stderr , "\n");
  for(i=0; i<rows; i++) {
    fprintf(stderr, " [");
...
void hungarian_print_costmatrix(hungarian_problem_t* p) {
   hungarian_print_matrixFloat(p->cost, p->num_rows, p->num_cols) ;
}
...
int hungarian_init(hungarian_problem_t* p, float** cost_matrix, int rows, int cols, int mode) {
